...
namespace CourierService.Core.Implementation
{
    public sealed class ShipmentPlanner : IShipmentPlanner
    {
        ...
        {
            if (remaining == null || remaining.Count == 0) return Array.Empty<Package>();

            var candidates = remaining.Where(p => p.WeightKg <= maxLoadKg).ToList();
            if (candidates.Count == 0) return Array.Empty<Package>();
            ...
            for (int i = 0; i < candidates.Count; i++)
            {
                ...
                {
                    if (!dp[w - p.WeightKg].HasValue) continue;

                    var prev = dp[w - p.WeightKg];

                    var cand = new PlanState(
                        hasValue: true,
                        count: prev.Count + 1,
                    ...
                    if (!dp[w].HasValue || Better(cand, dp[w]))
                        dp[w] = cand;
                }
            }
            ...
            for (int w = 1; w <= maxLoadKg; w++)
            {
                if (!dp[w].HasValue) continue;
                if (!best.HasValue || Better(dp[w], best))
                {
                    best = dp[w];
                    bestW = w;
                }
